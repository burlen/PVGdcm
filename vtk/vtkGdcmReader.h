...
public:
   ...
   int CanReadFile(const char *file)
   {
     ...
     if (fileName.find(".dcm") != string::npos) return 1;
     return 0;
   }
   ...
   vtkSetMacro(KeepOverlays, int);
   vtkGetMacro(KeepOverlays, int);
   vtkBooleanMacro(KeepOverlays, int);
...
/*
 * \ brief Sets the LoadMode as a intean string. 
 ...
 */   
   vtkSetMacro(DropDuplicatePositions, int);
   vtkGetMacro(DropDuplicatePositions, int);
   vtkBooleanMacro(DropDuplicatePositions, int);      
